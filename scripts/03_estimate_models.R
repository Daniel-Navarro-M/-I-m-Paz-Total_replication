...
mean_difference_interaction <- panel |>
  mutate(
    treatment = ifelse(treatment_dummy == 1, "Peace talks", "No talks"),
    clash     = ifelse(clash_dummy == 1, "Active dispute", "No dispute")
  ) |>
  group_by(treatment, clash) |>
  summarise(
    mean_count = mean(count),
    se         = sd(count) / sqrt(n()),
    .groups = "drop"
  ) |>
  ggplot(aes(x = treatment, y = mean_count, color = clash, group = clash)) +
  geom_pointrange(aes(ymin = mean_count - 1.96*se, ymax = mean_count + 1.96*se),
                  position = position_dodge(width = 0.3)) +
  geom_line(position = position_dodge(width = 0.3)) +
  labs(y = "Mean monthly events", x = NULL) +
  thesis_theme()

ggsave(mean_difference_interaction, 
       file = "output/figures/mean_difference_interaction.png", 
